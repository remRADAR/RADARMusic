...
function renderRelease() {
  ...
  document.querySelector('#identity-state').textContent = isPresave ? 'UPCOMING · PRE-SAVE' : 'NOW FEATURED';
  document.querySelector('#stream-kicker').textContent = isPresave ? 'PRE-SAVE · OFFICIAL FLOW' : 'STREAM · VERIFIED SOURCE';
...
document.querySelector('#sync-form').addEventListener('submit', async (event) => {
  ...
  try { const response = await fetch('/api/release/resolve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to resolve release.'); const sourceId = result.metadata.source === 'Spotify' ? new URL(result.metadata.sourceUrl).pathname.split('/').filter(Boolean).pop() : null; state.release = { ...state.release, ...result.metadata, destinations: result.stores.map((store) => ({ name: store.name, url: store.url, status: store.status === 'verified' ? 'verified' : 'review', type: store.type, id: store.name === 'Spotify' && result.metadata.mode !== 'presave' ? sourceId : null })) }; renderRelease(); openTab('stream'); resultNode.textContent = state.release.mode === 'presave' ? `Pre-save page matched for ${state.release.title}. Fans can continue through the verified campaign flow.` : `Matched ${state.release.title} by ${state.release.artist}. Review every destination before publishing.`; }

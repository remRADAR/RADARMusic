...
app.innerHTML = `
  <div class="welcome" id="welcome" role="dialog" aria-label="RADARMusic welcome animation" aria-modal="true"><img src="${opening}" alt="" /><div class="welcome__shade"></div><div class="welcome__mark"><span>THE</span><strong>RADAR<em>Music</em></strong></div><button id="skip-welcome" type="button">Enter</button></div>
  <main class="experience" data-tab="home">
    ...
    <nav class="tab-rail" aria-label="Release navigation"><button class="is-active" data-tab-target="home" type="button"><span>01</span>Home</button><button data-tab-target="stream" type="button"><span>02</span>Stream</button><button data-tab-target="youtube" type="button"><span>03</span>YouTube</button><button data-tab-target="profile" type="button"><span>04</span>Profile</button></nav>
    <section class="view view--home is-active" data-view="home"><div class="view__intro"><p class="kicker">RADAR ARTIST · CURRENT SIGNAL</p><h1 id="home-artist">${state.release.artist}</h1><p class="display-italic" id="home-title">${state.release.title}</p><div class="meta-line" id="home-meta">${state.release.type} · ${state.release.label} · ${state.release.date}</div><button class="primary-action" data-open-tab="stream" type="button">Enter the release <span>↗</span></button></div><div class="identity-card"><img id="identity-art" src="${state.release.artwork}" alt="${state.release.artist} release artwork" /><div><span>NOW FEATURED</span><strong id="identity-title">${state.release.title}</strong><small id="identity-artist">${state.release.artist}</small></div><button data-open-tab="stream" type="button" aria-label="Open stream tab">▶</button></div></section>
    <section class="view view--stream" data-view="stream"><div class="view__intro"><p class="kicker">STREAM · VERIFIED SOURCE</p><h2 id="stream-title">${state.release.title}</h2><p class="display-italic">Everywhere out.</p><p class="view__copy">The current release stays central while every approved listening destination sits one step away.</p></div><div class="content-panel stream-panel"><div class="embed-frame" id="stream-embed"></div><div class="store-strip" id="store-strip"></div></div></section>
...
function renderRelease() {
  ...
  document.querySelector('#home-meta').textContent = `${r.type || 'Release'} · ${r.label || 'Independent'} · ${r.date || 'Now'}`;
  document.querySelector('#identity-art').src = r.artwork || icon;
  ...
  document.querySelector('#stream-embed').innerHTML = source?.name === 'Spotify' && source.id ? `<iframe title="${escapeHTML(r.title)} on Spotify" src="https://open.spotify.com/embed/track/${encodeURIComponent(source.id)}?utm_source=generator&theme=0" loading="lazy" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>` : `<div class="source-fallback"><span>VERIFIED SOURCE</span><strong>${escapeHTML(source?.name || 'Music store')}</strong><a href="${source?.url || '#'}" target="_blank" rel="noreferrer">Open source ↗</a></div>`;
  document.querySelector('#store-strip').innerHTML = r.destinations.map((store) => `<a href="${store.url}" target="_blank" rel="noreferrer"><span>${escapeHTML(store.name.slice(0,1))}</span><strong>${escapeHTML(store.name)}</strong><small>${store.status === 'verified' ? 'Verified' : 'Review match'}</small></a>`).join('');
...
document.querySelector('#sync-form').addEventListener('submit', async (event) => {
  event.preventDefault(); const resultNode = document.querySelector('#sync-result'); const url = new FormData(event.currentTarget).get('url'); resultNode.textContent = 'Reading release metadata and matching stores…';
  try { const response = await fetch('/api/release/resolve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to resolve release.'); const sourceId = result.metadata.source === 'Spotify' ? new URL(result.metadata.sourceUrl).pathname.split('/').filter(Boolean).pop() : null; state.release = { ...state.release, ...result.metadata, destinations: result.stores.map((store) => ({ name: store.name, url: store.url, status: store.status === 'verified' ? 'verified' : 'review', id: store.name === 'Spotify' ? sourceId : null })) }; renderRelease(); openTab('stream'); resultNode.textContent = `Matched ${state.release.title} by ${state.release.artist}. Review every destination before publishing.`; }
